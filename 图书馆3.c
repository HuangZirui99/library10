...
registstu bb()      //将文件中的内容读出到链表中，返回值为表头地址 
{
	FILE *fp;       //文件指针 
	int n=0;
	registstu head=NULL;
	registstu p2,p,pr=NULL;
	fp=fopen("registstu","ab+");     //以只读的方式打开文件 
	while(!feof(fp))        //判断文件位置标志是否移动到文件末尾 
	{
	   n++;
		p=(registstu)malloc(N); //向内存申请一段空间 
	   fread(p,N,1,fp);     //将fp所指向的文件中的内容赋给p 
	   if(n==1)
	   {
	      head=p;
...
void del_book()   //删除信息 
{
	do
	{
		...
		while(p!=NULL)
		{
			if(p!=NULL&&strcmp(p->name,name)==0)          //P->name=name 的时候返回0 
			{
				j++;
...
{
	registstu p;
	p=(registstu)malloc(sizeof(N));     //申请空间
	system("cls");
	...
	scanf("%s",&p->password);
	FILE *fp;    //文件指针 
	fp=fopen("registstu","ab");   //以追加的方式打开名字为mybook的二进制文件 
	if(fp==NULL)
	{
		printf("cannot open file");
	}
...
void stulogin()
{
	...
	while(p!=NULL)
		{
			if(p!=NULL&&strcmp(p->registname,name)==0&&strcmp(p->password,password)==0)   //P->name=name 的时候返回0 
			{
				flag=1;
			}
			p=p->next;
